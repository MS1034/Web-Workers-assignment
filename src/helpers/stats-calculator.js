...
export default function calculateStatistics(paragraph) {
  const stats = {
...
export function calculateWordFrequency(paragraph, tWord) {
  const words = paragraph.split(/\s+/);
...
export function calculateAlphabetFrequency(paragraph) {
  const cleanedParagraph = paragraph.replace(/[^a-zA-Z]/g, "").toLowerCase();
